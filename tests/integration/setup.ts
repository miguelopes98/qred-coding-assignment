...
async function seedDatabase(): Promise<void> {
  ...
  const larsCard = await prisma.card.create({
    data: {
      employeeId: lars.id,
      status: CardStatus.ACTIVE,
      market: Market.SWEDEN,
  ...
  const categories = ['Office', 'Travel', 'Software', 'Meals', 'Marketing'];
  const descriptions = [
    'Office supplies',
    'Flight to Stockholm',
    'GitHub subscription',
  ...
  seedIds = {
    ...
    erikId: erik.id,
    larsId: lars.id,
    annasCardId: annasCard.id,
    eriksCardId: eriksCard.id,
    larsCardId: larsCard.id,
